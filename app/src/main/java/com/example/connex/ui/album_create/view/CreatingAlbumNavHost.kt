...
@Composable
fun CreatingAlbumNavHost(navController: NavHostController, applicationState: ApplicationState) {
    ...
    NavHost(navController = navController, startDestination = Constants.ALBUM_CREATING_GRAPH ) {
        navigation(route = Constants.ALBUM_CREATING_GRAPH, startDestination = Constants.ALBUM_CREATING_SELECT_FRIEND_ROUTE) {
            composable(route = Constants.ALBUM_CREATING_SELECT_FRIEND_ROUTE) { entry ->
                val backStackEntry = rememberNavControllerBackEntry(
                    entry = entry,
                    navController = applicationState.navController,
                    graph = Constants.HOME_GRAPH
                )
                FriendSelectScreen(albumCreatingViewModel = hiltViewModel(backStackEntry)) {applicationState.showSnackbar(it)}
            }
            composable(route = Constants.ALBUM_CREATING_SELECT_BACKGROUND_ROUTE) {
                BackgroundSelectScreen()
            }
            composable(route = Constants.ALBUM_CREATING_SETTING_NAME_ROUTE) {entry ->
                val backStackEntry = rememberNavControllerBackEntry(
                    entry = entry,
                    navController = applicationState.navController,
                    graph = Constants.HOME_GRAPH
                )
                AlbumNameSettingScreen(albumCreatingViewModel = hiltViewModel(backStackEntry))
            }
        }
    }
}